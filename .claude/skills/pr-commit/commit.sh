...
if [ "$CURRENT_BRANCH" = "main" ] || [ "$CURRENT_BRANCH" = "master" ]; then
...

  {
    ...
    echo "=== Staged diff ==="
    git diff --cached
  } | CLAUDE_HISTORY_ROLE=0 claude -p "$BRANCH_PROMPT" | tr -d '\r' | sed -e '/^```[a-zA-Z]*$/d' -e '/^```$/d' -e '/^$/d' | head -n 1 > "$BRANCH_NAME_FILE"
...

{
  ...
  echo "=== Staged diff ==="
  git diff --cached
} | CLAUDE_HISTORY_ROLE=0 claude -p "$PROMPT" | sed -e '/^```[a-zA-Z]*$/d' -e '/^```$/d' > "$COMMIT_MSG_FILE"
