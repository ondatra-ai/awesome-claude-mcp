...
**Always continue to the next numbered step after a nested skill returns — regardless of its result.** Step 5 (`update-memory`) and Step 9 (`pr-update`) invoke other skills. Outcomes like "no changes needed", "nothing to update", "already up to date", or any other "no-op" return are NOT stopping conditions — they mean the nested skill finished successfully and you MUST immediately proceed to the next step of THIS pipeline. Do not summarize the nested skill's result, do not pause for confirmation, do not ask the user what to do — just run the next step. The pipeline only ends when Step 10's execution report has been emitted.
